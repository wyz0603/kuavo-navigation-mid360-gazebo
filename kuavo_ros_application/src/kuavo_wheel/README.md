...
python3 src/kuavo_wheel/scripts/vehicle_bridge.py
```
...
### 查看帮助信息
```bash
python3 src/kuavo_wheel/scripts/vehicle_bridge.py --help
```
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--kuavo-master` | `http://192.168.26.1:11311` | Kuavo系统的ROS Master URI |
| `--wheel-master` | `http://169.254.128.2:11311` | Wheel系统的ROS Master URI |
| `--kuavo-slave-ip` | `192.168.26.12` | Kuavo从机的IP地址 |
| `--wheel-slave-ip` | `169.254.128.109` | Wheel从机的IP地址 |
...
```bash
# 设置kuavo master URI
export KUAVO_MASTER_URI="http://192.168.26.1:11311"

# 设置kuavo slave IP
export KUAVO_SLAVE_IP="192.168.26.12"
...
src/kuavo_wheel/
├── scripts/
│   ├── vehicle_bridge.py                    # 主程序
│   ├── start_vehicle_bridge.sh             # 启动脚本（自动检测网络和环境变量）
