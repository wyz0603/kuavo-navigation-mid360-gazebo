...
def kuavo_process(master_uri, shared_data, kuavo_slave_ip):
    ...
    try:
        ...
        print("[Kuavo Process] Subscribed to /cmd_vel")
            ...
            try:
                ...
                with shared_data['cmd_vel_lock']:
                    if shared_data['cmd_vel_data'] is not None:
                        cmd_vel_pub.publish(shared_data['cmd_vel_data'])
                        # print(shared_data['cmd_vel_data'])
                # print(f"[Wheel Process] Sent cmd_vel to /base_cmd_vel")
            except Exception as e:
                # 队列为空或其他错误，继续循环
                pass
            rate.sleep()
...
def main():
    ...
    parser.add_argument('--kuavo-master', default='http://192.168.26.1:11311', 
                       help='Kuavo ROS Master URI (default: http://192.168.26.1:11311)')
    ...
    parser.add_argument('--kuavo-slave-ip', default='192.168.26.12', 
                       help='Kuavo slave IP address (default: 192.168.26.12)')
    parser.add_argument('--wheel-slave-ip', default='169.254.128.109', 
                       help='Wheel slave IP address (default: 169.254.128.109)')
