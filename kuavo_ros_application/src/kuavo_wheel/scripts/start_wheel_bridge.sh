...
if [[ -z "$KUAVO_SLAVE_IP" ]]; then
    # 尝试从ROS_IP获取
    if [[ -n "$ROS_IP" ]]; then
        KUAVO_SLAVE_IP="$ROS_IP"
        echo "Using ROS_IP as KUAVO_SLAVE_IP: $KUAVO_SLAVE_IP"
    else
        KUAVO_SLAVE_IP="192.168.26.12"
        echo "KUAVO_SLAVE_IP not set, using default: $KUAVO_SLAVE_IP"
    fi
else
    echo "Using KUAVO_SLAVE_IP: $KUAVO_SLAVE_IP"
fi
...
detect_wheel_network() {
    ...
    WHEEL_SLAVE_IP="169.254.128.109"
    return 0
}
