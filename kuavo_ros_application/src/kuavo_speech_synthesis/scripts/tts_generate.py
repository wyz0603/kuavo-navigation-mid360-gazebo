...
class TTSGenerator:
    ...
    def load_model(self):
        try:
            print("TTSGenerator Loading TTS Model: " + self._model_id)
            start_time = time.time()
            os.environ["CUDA_VISIBLE_DEVICES"] = "-1,"
            self._sambert_hifigan_tts = pipeline(task=Tasks.text_to_speech,  device='cpu', model=self._model_id)
            end_time = time.time()
            load_time = end_time - start_time
            print(f"TTSGenerator Loading model time cost: {load_time:.2f} seconds")
            return True
        except Exception as e:
            print(f"TTSGenerator Loading model error: {e}")
            self._sambert_hifigan_tts = None 
            return False
    def generate_text_to_speech(self, text:str, filepath:str)->bool:
        ...
        if self._sambert_hifigan_tts is None:
            if not self.load_model():
                return  False
        try:
            print(f"TTSGenerator Generating TTS for: {text}")
            start_time = time.time()
            ...
            end_time = time.time()
            load_time = end_time - start_time
            print(f"TTSGenerator Generating TTS time cost: {load_time:.2f} seconds")
            ...
            print(f"TTSGenerator generate Error: {e}")  
