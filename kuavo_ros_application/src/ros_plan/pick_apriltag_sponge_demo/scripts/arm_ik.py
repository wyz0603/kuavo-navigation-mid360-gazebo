...
class ArmIk:
    ...
    def computeArmIK(self, start_q, l_ps:PoseStamped, r_ps:PoseStamped):
        l_hand_pose = None
        l_hand_RPY = None
        r_hand_pose = None
        r_hand_RPY = None
        ...
        if l_ps is not None:
            l_hand_pose = [l_ps.pose.position.x, l_ps.pose.position.y, l_ps.pose.position.z]
            x = l_ps.pose.orientation.x
            y = l_ps.pose.orientation.y
            z = l_ps.pose.orientation.z
            w = l_ps.pose.orientation.w
            l_hand_RPY = R.from_quat([x,y,z,w]).as_euler('xyz', degrees=False)
        if r_ps is not None:
            r_hand_pose = [r_ps.pose.position.x, r_ps.pose.position.y, r_ps.pose.position.z]
            x = r_ps.pose.orientation.x
            y = r_ps.pose.orientation.y
            z = r_ps.pose.orientation.z
            w = r_ps.pose.orientation.w
