...
class YoloTransform:
    def __init__(self):
        ...
        # 订阅检测结果话题
        self.detection_sub = rospy.Subscriber('/yolov8_detections', Detection2DArray, self.detection_callback)

        # 发布转换后的结果话题
        self.transformed_pub = rospy.Publisher('/robot_yolov8_info', Detection2DArray, queue_size=10)
    ...
    def detection_callback(self, msg):
        try:
            # 查找坐标变换
            transform = self.tf_buffer.lookup_transform('base_link', 'camera_color_optical_frame', rospy.Time(0))
        ...
        for detection in msg.detections:
            ...
            transform_stamped.child_frame_id = f"robot_yolov8_object_{detection.results[0].id}"
