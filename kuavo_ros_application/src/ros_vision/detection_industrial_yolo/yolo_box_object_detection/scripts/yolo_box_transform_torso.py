...
class YoloTransform:
    def __init__(self):
        # 初始化节点
        rospy.init_node('yolo_box_transform', anonymous=True)

        # 订阅检测结果话题
        self.detection_sub = rospy.Subscriber('/object_yolo_box_segment_result', Detection2DArray, self.detection_callback)

        # 发布转换后的结果话题
        self.transformed_pub = rospy.Publisher('/object_yolo_box_tf2_torso_result', Detection2DArray, queue_size=10)
    ...
    def detection_callback(self, msg):
        ...
        for detection in msg.detections:
            ...
            transformed_detection.results[0].pose.pose.orientation.w = 0.5 
            transformed_detection.results[0].pose.pose.orientation.x = 0.5
            transformed_detection.results[0].pose.pose.orientation.y = -0.5
            transformed_detection.results[0].pose.pose.orientation.z = -0.5
            # transformed_detection.results[0].pose.pose.orientation.w = 0.5
            # transformed_detection.results[0].pose.pose.orientation.x = 0.6
            # transformed_detection.results[0].pose.pose.orientation.y = 0.605
            # transformed_detection.results[0].pose.pose.orientation.z = -0.170

            # 创建TransformStamped消息用于TF广播
            transform_stamped = TransformStamped()
            transform_stamped.header.stamp = rospy.Time.now()
            transform_stamped.header.frame_id = 'torso'
            transform_stamped.child_frame_id = f"torso_box_object_{detection.results[0].id}"
