...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    // Skip plugin-side TF publishing: when the lidar is attached to a robot
    // URDF, robot_state_publisher already provides the parent_link -> sensor_link
    // chain. Republishing here gives livox_link two parents in the TF tree and
    // tf2::lookupTransform returns inconsistent rotations, which breaks any
    // downstream code that relies on the lidar's URDF-defined mounting.
...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        // Disabled: see comment in Load(). robot_state_publisher owns the
        // parent_link -> livox_link TF via the URDF chain.
        ...
        scan_point.header.stamp = ros::Time().fromSec(world->SimTime().Double());
...
void LivoxPointsPlugin::SendRosTf(const ignition::math::Pose3d &pose, const std::string &father_frame,
    ...
    const ros::Time stamp = world ? ros::Time().fromSec(world->SimTime().Double()) : ros::Time::now();
    tfBroadcaster->sendTransform(tf::StampedTransform(tf, stamp, father_frame, child_frame));
}
