...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='单步行走步数')
    ...
    # ---- 加入退出机制 ----
    total_time = msg.timeTrajectory[-1] * 2.5   # 获取总执行时间
    rospy.loginfo(f"动作执行大约需要 {total_time:.2f} 秒，等待执行完成...")

    rospy.sleep(total_time + 0.5)         # 加 0.5 秒冗余
    rospy.loginfo("动作执行完成，退出程序。")