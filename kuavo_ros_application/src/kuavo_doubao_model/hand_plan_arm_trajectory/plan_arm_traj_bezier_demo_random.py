...
# 构建 action_files 路径
tact_dir = os.path.join(current_dir, 'action_files')

def get_random_tact_file():
    """获取指定目录下随机的.tact文件路径"""
    try:
        # 遍历目录，筛选所有.tact文件
        tact_files = [
            f for f in os.listdir(tact_dir)
            if f.endswith('.tact') and os.path.isfile(os.path.join(tact_dir, f))
        ]
        if not tact_files:
            # 无.tact文件时，抛出异常或返回默认值（按需调整）
            raise FileNotFoundError(f"目录 {tact_dir} 下未找到.tact文件")
        # 随机选择一个.tact文件
        random_file = random.choice(tact_files)
        # 拼接完整文件路径
        return os.path.join(tact_dir, random_file)
    except Exception as e:
        rospy.logwarn(f"获取随机.tact文件失败：{e}")
        # 异常时返回原默认文件（兜底）
        return '/home/leju_kuavo/kuavo_ros_application/src/kuavo_doubao_model/hand_plan_arm_trajectory/action_files/打招呼_45.tact'
...
def main():
    ...
    # 加载动作文件
    file_path = rospy.get_param('~tact_file', get_random_tact_file())
