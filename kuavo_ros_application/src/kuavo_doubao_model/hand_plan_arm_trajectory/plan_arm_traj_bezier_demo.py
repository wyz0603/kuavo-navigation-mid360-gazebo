...
from kuavo_msgs.srv import stopPlanArmTrajectory
...
def main():
    ...
    """
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='执行手臂动作轨迹')
    parser.add_argument('--act', type=str, default='打招呼_45.tact', 
                       help='要执行的动作文件名 (默认: 打招呼_45.tact)')
    args = parser.parse_args(rospy.myargv()[1:])  # 使用rospy.myargv()处理ROS参数
    
    # 初始化ROS节点
    rospy.init_node('arm_trajectory_bezier_demo')
    global start_frame_time, end_frame_time, x_shift
    
    print(f"[动作] 执行动作文件: {args.act}")
    ...
    # 构建动作文件的相对路径
    action_file_path = os.path.join(current_dir, 'action_files', args.act)
    
    # 检查文件是否存在
    if not os.path.exists(action_file_path):
        rospy.logerr(f"动作文件不存在: {action_file_path}")
        rospy.loginfo("可用的动作文件:")
        action_files_dir = os.path.join(current_dir, 'action_files')
        if os.path.exists(action_files_dir):
            for file in os.listdir(action_files_dir):
                if file.endswith('.tact'):
                    rospy.loginfo(f"  - {file}")
        return
    
    # 加载动作文件
    file_path = action_file_path
    start_frame_time, end_frame_time = get_start_end_frame_time(file_path)
    end_frame_time = end_frame_time + 1
    x_shift = start_frame_time - 1
    data = load_json_file(file_path)
    if not data:
        rospy.logerr(f"无法加载动作文件: {file_path}")
        return
    ...
    while not rospy.is_shutdown():
        try:
            ...
            ###### 自动退出机制：轨迹执行完成后退出 ######
            if rospy.get_time() - start_time > end_frame_time:
                rospy.loginfo("Trajectory finished. Exiting...")
                call_change_arm_ctrl_mode_service(1)
                rospy.signal_shutdown("Trajectory done")
                break
