...
from lib import protocol, config
...
class RealtimeDialogClient:
    ...
    async def start_connection(self) -> bool:
        ...
        try:
            ...
            print(f"StartConnection response: {parsed_response}")
            ...
            if e.status_code == 401:
                print(f"WebSocket authentication failed: HTTP {e.status_code} - App ID or Access Key error")
            else:
                print(f"WebSocket connection failed: HTTP {e.status_code} - {e}")
            return False
