...
install_zsh() {
    ...
    if [[ ! -d "$HOME/.oh-my-zsh" ]]; then
        if ! timeout 60 git clone https://github.com/ohmyzsh/ohmyzsh.git ~/.oh-my-zsh; then
            log_warning "GitHub连接失败，跳过oh-my-zsh安装"
            log_success "zsh安装完成"
            return 0
        fi
    fi
    ...
    if [[ ! -d "$ZSH_CUSTOM/plugins/zsh-autosuggestions" ]]; then
        timeout 60 git clone https://github.com/zsh-users/zsh-autosuggestions.git "$ZSH_CUSTOM/plugins/zsh-autosuggestions" || \
            log_warning "GitHub连接失败，跳过zsh-autosuggestions插件"
    fi
    
    if [[ ! -d "$ZSH_CUSTOM/plugins/zsh-syntax-highlighting" ]]; then
        timeout 60 git clone https://github.com/zsh-users/zsh-syntax-highlighting.git "$ZSH_CUSTOM/plugins/zsh-syntax-highlighting" || \
            log_warning "GitHub连接失败，跳过zsh-syntax-highlighting插件"
    fi
    
    # 配置zsh，避免依赖额外下载的zsh-in-docker脚本。
    cat > "$HOME/.zshrc" <<'EOF'
...
configure_environment() {
    ...
    "${SCRIPT_DIR}/install_openvino.sh"
    "${SCRIPT_DIR}/config_ccache.sh"
    configure_docker
